...
namespace hp55games.MapGame.Features.HexDebugAdapter.Editor
{
    ...
    public static class MapGameRegistryPopulator
    {
        public static void Populate(HexGridRegistry registry)
        {
            ...
            if (controller == null)
            {
                Debug.LogWarning("[HexDebug] Nessun HexGridController trovato in scena. Impossibile popolare il registry.");
                return;
            }
            ...
            foreach (var kvp in controller.Tiles)
            {
                ...
                if (!worldPositions.TryGetValue(coord, out var worldPos))
                {
                    ...
                    Debug.LogWarning($"[HexDebug] HexTileView mancante per coord {coord}. WorldPosition impostata a zero.");
                }
